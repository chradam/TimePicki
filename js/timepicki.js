...
(function ($) {
    ...
    $.fn.timepicki = function (options) {
        ...
        return this.each(function () {
            var ele = $(this);
            var ele_hei = -Math.abs(ele.outerHeight());
            ele_hei -= 73;
            ...
            if (settings.show_hour && settings.show_minute) {
                new_ele.append("<span>:</span>");
            }
            ...
            $(document).on("click", function (event) {
                if (!$(event.target).is(ele_next) && (ele_next.css("display") === "block" || ele_next.css("display") === "flex") && !$(event.target).is($('.reset_time'))) {
                    var ele_lef = 0;
            ...
            inputs.on('keydown', function (e) {
                ...
                if (input.closest('.timepicker_wrap .time').length && settings.show_hour) {
                    change_time(null, direction);
                } else if (input.closest('.timepicker_wrap .mins').length && settings.show_minute) {
            ...
            function set_value(event, close) {
                ...
                var tim = "00";
                if (settings.show_hour) {
                    tim = ele_next.find(".ti_tx input").val();
                }

                var mini = "00";
                if (settings.show_minute) {
                    mini = ele_next.find(".mi_tx input").val();
                }
            ...
            function set_date(start_time) {
                ...
                if (ele.is('[data-timepicki-tim]')) {
                ...
                } else if (typeof start_time === 'object' && start_time.length > 0) {
                    if (settings.show_hour) {
                    ...
                    if (settings.show_meridian && start_time.length > 2) {
                        mer = start_time[2];
                    }
